...
function apriTabGiochi(tabId) {
  document.querySelectorAll("#giochi-paziente .tab-btn").forEach(b => b.classList.toggle("attivo", b.dataset.tab === tabId));
  document.querySelectorAll("#giochi-paziente .tab-pannello").forEach(p => p.classList.toggle("hidden", p.id !== tabId));
}

// ---------- Giochi (paziente): quiz fake news, abbina categorie, componi il pasto ----------
// Nessun dato viene salvato: sono giochi didattici, il punteggio esiste solo per la sessione corrente.
...
function renderGiochiQuiz() {
  if (giochiQuizIndice >= giochiQuizOrdine.length) {
    giochiQuizContenuto.innerHTML = `
      ...
      <button type="button" class="secondary" data-azione="quiz-ricomincia">Rigioca</button>
    `;
    return;
  }
  ...
  giochiQuizContenuto.innerHTML = `
    ...
    <div class="giochi-quiz-bottoni">
      <button type="button" class="secondary" data-azione="quiz-rispondi" data-valore="true">✅ Vero</button>
      <button type="button" class="secondary" data-azione="quiz-rispondi" data-valore="false">❌ Falso</button>
    </div>
...
function rispondiQuiz(valoreScelto) {
  ...
  feedback.innerHTML = `
    ...
    <button type="button" data-azione="quiz-avanti">Prossima domanda →</button>
  `;
}
...
function renderGiochiAbbina() {
  ...
  giochiAbbinaContenuto.innerHTML = `
    ...
    ${punteggioHtml}
    <div class="giochi-abbina-azioni">
      ${!giochiAbbinaVerificato
        ? `<button type="button" data-azione="abbina-verifica" ${nonAssegnati.length ? "disabled" : ""}>Verifica abbinamenti</button>`
        : `<button type="button" class="secondary" data-azione="abbina-ricomincia">Rigioca</button>`}
    </div>
  `;
}
...
const GIOCHI_PASTO_OPZIONI = [
  { nome: "Petto di pollo alla griglia", kcal: 220, categoria: "proteine" },
  { nome: "Salmone al forno", kcal: 260, categoria: "proteine" },
  { nome: "Riso integrale", kcal: 250, categoria: "carboidrati" },
  { nome: "Pane integrale", kcal: 180, categoria: "carboidrati" },
  { nome: "Insalata mista", kcal: 30, categoria: "verdura" },
  { nome: "Verdure grigliate", kcal: 60, categoria: "verdura" },
  { nome: "Olio extravergine d'oliva (1 cucchiaio)", kcal: 90, categoria: "grassi" },
  { nome: "Frutta fresca di stagione", kcal: 70, categoria: "frutta" },
  { nome: "Patatine fritte", kcal: 320, categoria: "junk" },
  { nome: "Bibita zuccherata", kcal: 150, categoria: "junk" },
  { nome: "Merendina confezionata", kcal: 250, categoria: "junk" },
  { nome: "Maionese abbondante", kcal: 180, categoria: "junk" },
];

let giochiPastoSelezionati = new Set();
let giochiPastoVerificato = false;

function valutaPasto(selezionati, totaleKcal) {
  ...
  if (totaleKcal < 350) messaggi.push("⚠️ Il pasto sembra piuttosto leggero per un pranzo completo.");
  else if (totaleKcal > 800) messaggi.push("⚠️ Il totale calorico è piuttosto alto per un singolo pasto.");
  else messaggi.push("✅ L'apporto calorico è in un intervallo ragionevole per un pasto principale.");

  const positivo = haProteine && haCarboidrati && haVerduraFrutta && junk.length === 0 && totaleKcal >= 350 && totaleKcal <= 800;
...
function renderGiochiPasto() {
  const opzioniHtml = GIOCHI_PASTO_OPZIONI.map(op => {
    ...
    return `<button type="button" class="giochi-chip ${classeExtra}" data-azione="pasto-toggle" data-nome="${escapeHtml(op.nome)}" ${giochiPastoVerificato ? "disabled" : ""}>${escapeHtml(op.nome)} <span class="giochi-kcal">${op.kcal} kcal</span></button>`;
  }).join("");

  const selezionati = GIOCHI_PASTO_OPZIONI.filter(o => giochiPastoSelezionati.has(o.nome));
  const totaleKcal = selezionati.reduce((s, o) => s + o.kcal, 0);

  let valutazioneHtml = "";
  if (giochiPastoVerificato) {
    const { messaggi, positivo } = valutaPasto(selezionati, totaleKcal);
  ...
  giochiPastoContenuto.innerHTML = `
    <p class="hint">Scegli gli alimenti per comporre un pranzo equilibrato, poi verifica il risultato.</p>
    <div class="giochi-pasto-opzioni">${opzioniHtml}</div>
    <p class="giochi-pasto-totale">Totale: <strong>${totaleKcal} kcal</strong></p>
    ${valutazioneHtml}
    <div class="giochi-abbina-azioni">
      ${!giochiPastoVerificato
        ? `<button type="button" data-azione="pasto-verifica" ${selezionati.length === 0 ? "disabled" : ""}>Verifica il pasto</button>`
        : `<button type="button" class="secondary" data-azione="pasto-ricomincia">Nuovo pasto</button>`}
    </div>
  `;
}
...
function inizializza() {
  ...
  document.querySelectorAll("#giochi-paziente .tab-btn").forEach(btn => {
    btn.addEventListener("click", () => apriTabGiochi(btn.dataset.tab));
  });
